...
def process_pods(pods: list[Any], pool: dict[Volume, str], pvcs: list[Any], ns: str, new_pool_keys: set[Volume]):
    for pod in pods:
        ...
        try:
            ...
            logger.debug(
                list(
                    map(
                        lambda voc: voc['persistent_volume_claim']['claim_name'],
                        list(filter(lambda vocl: vocl['persistent_volume_claim'], pod['spec']['volumes']))
                    )
                )
            )
            for vc in pod['spec']['volumes']:
                if vc['persistent_volume_claim']:
                    logger.debug(f"claim: {vc['persistent_volume_claim']['claim_name']}")
                    process_pvc(
...
def process_pvc(pvc: str, pvcs: list[Any], pod_name: str, pool: dict[Volume, str], ns: str, new_pool_keys: set[Volume]):
    for v in pvcs:
        logger.debug(f"vol name: {v['metadata']['name']}")
        logger.debug(f'pvc: {pvc}')
        if v['metadata']['name'] == pvc:
            vol_name = v['spec']['volume_name']
            logger.info("NS: %s, POD: %s, VOLUME: %s, PVC: %s" % (ns, pod_name, vol_name, pvc))
...
class KubeClientImpl(KubeClient):
    ...
    def list_namespaced_persistent_volume_claim(self, ns: str) -> list[Any]:
        return get_items(self.k8s_api_client.list_namespaced_persistent_volume_claim(ns))
    ...
    def get_single_namespace_data(self, ns: str) -> PodsPvcs:
        ...
        logger.debug('pvcs:')
        logger.debug(list(map(lambda v: (v['metadata']['name'], v['spec']['volume_name']), pvcs)))
        return PodsPvcs(pods=pods, pvcs=pvcs)


def main(argv: list[str] = None, kube_client: KubeClient = KubeClientImpl()):
    ...
    while 1:
        ...
        old_pool_keys = cleanup_pool(pool=pool, old_pool_keys=old_pool_keys, new_pool_keys=new_pool_keys)
